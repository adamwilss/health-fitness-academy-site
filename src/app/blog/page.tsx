          ...
          <div className="space-y-6">
            {blogPosts.map((post, i) => (
              <Reveal key={post.slug} delay={i * 100}>
                <a
                  href={`/blog/${post.slug}`}
                  className="group block rounded-2xl border border-line bg-card p-7 transition-colors hover:border-brand/50 sm:p-9"
                >
                  <p className="mb-3 font-mono text-xs uppercase tracking-[0.16em] text-muted">
                    {post.displayDate} &middot; {post.readingTime}
                  </p>
                  <h2 className="mb-3 font-heading text-2xl font-semibold leading-snug text-ink">
                    {post.title}
                  </h2>
                  <p className="mb-5 text-base leading-relaxed text-muted">{post.excerpt}</p>
